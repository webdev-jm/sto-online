...
class InventoryDetails extends Component
{
    ...
    public function render()
    {
        ...
        $inventories = Inventory::leftJoin(env('DB_DATABASE_2').'.products as p', 'p.id', '=', 'inventories.product_id')
            ->where('inventories.account_id', $this->account->id)
            ->where('inventories.inventory_upload_id', $this->inventory_upload->id)
            ->paginate(15, ['inventories.*', 'p.stock_code', 'p.description'], 'inventory-page');
