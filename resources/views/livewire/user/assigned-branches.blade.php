<div>
    <style>
        ...
        .empty-state-inline small { font-size: 0.72rem; opacity: 0.65; display: block; margin-top: 4px; }
    </style>
    ...
    <div class="card mb-2">
        ...
        <div class="card-body">
            ...
            <div class="d-flex align-items-center mb-3" style="gap: 6px;">
                ...
                >
                    <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="9 11 12 14 22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2h11"/></svg>
                    Select All
                </button>
                @if(!empty($selected))
            ...
            <div class="row" style="row-gap: 6px;">
                @forelse($branches as $branch)
                    @php $isSelected = !empty($selected) && in_array($branch->id, $selected); @endphp
                    <div class="col-lg-6">
                        <button
                            class="item-btn {{ $isSelected ? 'btn btn-primary is-selected' : 'btn btn-default' }}"
                            wire:loading.attr="disabled"
                            wire:click.prevent="selectBranch('{{ encrypt($branch->id) }}')"
                        >
                            <span class="item-code">{{ $branch->code }}</span>
                ...
                @empty
                    <div class="col-12">
                        <div class="empty-state-inline">
                            <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" style="display:block;margin:0 auto 10px;"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
                            <p>No branches available</p>
                            <small>Assign accounts first to see branches</small>
                        </div>
                    </div>
                @endforelse
            </div>
        ...
        <div class="card-footer d-flex justify-content-end">
            ...
            >
                <span wire:loading.remove wire:target="assign">Save Branches</span>
